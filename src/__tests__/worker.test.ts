...
describe("Cloudflare Worker entrypoint", () => {
  ...
  it("handles MCP initialize", async () => {
    ...
    expect(body.result?.serverInfo?.name).toBe("xero-mcp");
  });
  ...
  it("lists all tools without credentials", async () => {
    ...
    expect(names).toContain("xero_navigate");
    expect(names).toContain("xero_status");
    expect(names).toContain("xero_contacts_list");
  ...
  it("returns a graceful error for a credential-requiring tool when unconfigured", async () => {
    const res = await mcp({
      ...
      id: 3,
      method: "tools/call",
      params: { name: "xero_contacts_list", arguments: {} },
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
    ...
    expect(body.result?.content?.[0]?.text).toMatch(
      /XERO_ACCESS_TOKEN|credentials|required/i,
    );
  });
